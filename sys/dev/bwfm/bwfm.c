...
int	 bwfm_chip_attach(device_t dev, struct bwfm_softc *);
#ifdef NOTYET
...
void
bwfm_attachhook(void *arg)
{
	struct bwfm_softc *sc = (struct bwfm_softc *)arg;

	device_printf(sc->sc_dev, "%s: called\n", __func__);
	if (BWFM_BUS_PREINIT(sc->sc_dev, sc))
		return;
#ifdef NOTYET
	if (bwfm_preinit(sc))
		return;
...
int
bwfm_chip_attach(device_t dev, struct bwfm_softc *sc)
{
	...
	if (BWFM_BUSCORE_PREPARE(dev, sc) != 0) {
		device_printf(sc->sc_dev, "failed buscore prepare\n");
		return 1;
	}

	val = BWFM_BUSCORE_READ(dev, sc,
	...
	device_printf(sc->sc_dev, "Chip name: %s\n", sc->sc_chip.ch_name);
	switch (BWFM_CHIP_CHIPID_TYPE(val))
	{
	case BWFM_CHIP_CHIPID_TYPE_SOCI_SB:
		device_printf(sc->sc_dev, "SoC interconnect SB not implemented\n");
		return 1;
	case BWFM_CHIP_CHIPID_TYPE_SOCI_AI:
		...
		break;
	default:
		device_printf(sc->sc_dev, "SoC interconnect %d unknown\n",
		    BWFM_CHIP_CHIPID_TYPE(val));
		return 1;
	}
	...
	if (!cpu_found) {
		device_printf(sc->sc_dev, "CPU core not detected\n");
		return 1;
	}
	if (need_socram && !has_socram) {
		device_printf(sc->sc_dev, "RAM core not provided\n");
		return 1;
	}

	BWFM_BUSCORE_RESET(sc->sc_dev, sc);
	...
	sc->sc_chip.ch_cc_caps = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	    core->co_base + BWFM_CHIP_REG_CAPABILITIES);
	sc->sc_chip.ch_cc_caps_ext = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	...
	if (sc->sc_chip.ch_cc_caps & BWFM_CHIP_REG_CAPABILITIES_PMU) {
		sc->sc_chip.ch_pmucaps = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	...
	BWFM_BUSCORE_SETUP(sc->sc_dev, sc);
...
bwfm_chip_ai_isup(struct bwfm_softc *sc, struct bwfm_core *core)
{
	...
	ioctl = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	    core->co_wrapbase + BWFM_AGENT_IOCTL);
	reset = BWFM_BUSCORE_READ(sc->sc_dev, sc,
    ...
    uint32_t prereset, uint32_t reset)
{
	...
	val = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	    core->co_wrapbase + BWFM_AGENT_RESET_CTL);
	if ((val & BWFM_AGENT_RESET_CTL_RESET) == 0) {

		BWFM_BUSCORE_WRITE(sc->sc_dev,sc,
		    core->co_wrapbase + BWFM_AGENT_IOCTL,
		    prereset | BWFM_AGENT_IOCTL_FGC | BWFM_AGENT_IOCTL_CLK);
		BWFM_BUSCORE_READ(sc->sc_dev, sc,
		    core->co_wrapbase + BWFM_AGENT_IOCTL);
		...
		for (i = 300; i > 0; i--) {
			if (BWFM_BUSCORE_READ(sc->sc_dev, sc,
			    core->co_wrapbase + BWFM_AGENT_RESET_CTL) ==
			    BWFM_AGENT_RESET_CTL_RESET)
				break;
		}
		if (i == 0)
			device_printf(sc->sc_dev, "timeout on core reset\n");
	}
...
bwfm_chip_dmp_erom_scan(struct bwfm_softc *sc)
{
	...
	erom = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	    BWFM_CHIP_BASE + BWFM_CHIP_REG_EROMPTR);
	while (type != BWFM_DMP_DESC_EOT) {
		val = BWFM_BUSCORE_READ(sc->sc_dev, sc, erom);
		...
		if (type != BWFM_DMP_DESC_COMPONENT) {
			device_printf(sc->sc_dev, "not component descriptor\n");
			return;
		}
...
bwfm_chip_cr4_set_active(struct bwfm_softc *sc, uint32_t rstvec)
{
	...
	BWFM_BUSCORE_ACTIVATE(sc->sc_dev, sc, rstvec);
...
bwfm_chip_cm3_set_active(struct bwfm_softc *sc)
{
	...
	BWFM_BUSCORE_ACTIVATE(sc->sc_dev, sc, 0);
...
bwfm_chip_sr_capable(struct bwfm_softc *sc)
{
	...
	case BRCM_CC_4356_CHIP_ID:
		core = bwfm_chip_get_pmu(sc);
		BWFM_BUSCORE_WRITE(sc->sc_dev,sc, core->co_base +
		    BWFM_CHIP_REG_CHIPCONTROL_ADDR, 3);
		reg = BWFM_BUSCORE_READ(sc->sc_dev, sc, core->co_base +
...
bwfm_chip_socram_ramsize(struct bwfm_softc *sc, struct bwfm_core *core)
{
	...
	coreinfo = BWFM_BUSCORE_READ(sc->sc_dev, sc,
	...
	if (core->co_rev <= 7 || core->co_rev == 12) {
		...
		for (i = 0; i < nb; i++) {
			...
			bankinfo = BWFM_BUSCORE_READ(sc->sc_dev, sc,
...
bwfm_chip_tcm_ramsize(struct bwfm_softc *sc, struct bwfm_core *core)
{
	...
	cap = BWFM_BUSCORE_READ(sc->sc_dev, sc, core->co_base + BWFM_ARMCR4_CAP);
	...
	for (i = 0; i < totb; i++) {
		...
		bxinfo = BWFM_BUSCORE_READ(sc->sc_dev, sc,
...
bwfm_chip_tcm_rambase(struct bwfm_softc *sc)
{
	...
	case BRCM_CC_4378_CHIP_ID:
		...
		break;
	default:
		device_printf(sc->sc_dev, "unknown chip: %d\n",
		    sc->sc_chip.ch_chip);
		break;
	}
}
...
bwfm_loadfirmware(struct bwfm_softc *sc, const char *chip, const char *bus)
{
	...
	if (sysname != NULL) {
		r = snprintf(name, sizeof(name), "brcmfmac%s%s.%s", chip,
		    bus, sysname);
		if (r > 0 && r < sizeof(name)) {
			sc->fw_bin = firmware_get(name);
		}
	}
	if (sc->fw_bin == NULL) {
		snprintf(name, sizeof(name), "brcmfmac%s%s", chip, bus);
		if ((sc->fw_bin = firmware_get(name)) == NULL) {
			snprintf(name, sizeof(name), "brcmfmac%s%s%s%s", chip, bus,
	...
	if (sysname != NULL) {
		...
		if ((r > 0 && r < sizeof(name)) &&
			...
			if (bwfm_nvram_convert(*nvram, *nvsize, nvlen) != 0) {
				device_printf(sc->sc_dev, "failed to process file %s\n",
				    name);
